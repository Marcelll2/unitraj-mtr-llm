    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            print(f''
                  ...
                  f'data_usage_this_dataset: {data_usage_this_dataset} and data_chunk_size: {self.data_chunk_size}'
                  f'starting_frame: {self.starting_frame}'
                  f'')            
            if self.config['use_cache'] or is_ddp():
                ...
                if os.path.exists(self.cache_path) and self.config.get('overwrite_cache', False) is False:
                    ...
                    _, summary_list, mapping = read_dataset_summary(data_path) # summary_list是所有样本的文件名，ID
                    ...
                    data_splits = np.array_split(summary_list, process_num) # 按照进程数量进行数据阶段，平均分配给每个进行一定量的数据样本，故此有process_num个数据列表
                    ...
                    for i in range(process_num):
                        with open(os.path.join('tmp', '{}.pkl'.format(i)), 'wb') as f:
                            ...
                            # tmp{i}.pkl的内容是(data_path, mapping, list(data_splits[i]), dataset_name)
