...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        if not rm_self_attn_decoder: # sa: self attention
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            # query content multi head self attention
            # Apply projections here
            # shape: num_queries x batch_size x 256
            q_content = self.sa_qcontent_proj(tgt)  # target is the input of the first decoder layer. zero by default.
                                                    # tgt: query_content
            q_pos = self.sa_qpos_proj(query_pos) # query_pos: intention_query
            ...
            # Query Content Multi head self attention 图中的 add加法 操作
            q = q_content + q_pos
            k = k_content + k_pos
            ...
            # post operation of self attention
            tgt = tgt + self.dropout1(tgt2) # self attention 的 Add
            tgt = self.norm1(tgt) # self attention 的 Norm
        ...
        # Apply projections here
        # shape: num_queries x batch_size x 256
        q_content = self.ca_qcontent_proj(tgt) # Self-attention的输出tgt被转换特征维度用于下一个Cross attention module，生成q_content
        ...
        if self.use_local_attn and memory_valid_mask is not None:
            ...
            k_content = self.ca_kcontent_proj(memory) # 原文图中Multi head (Cross) Attention的context feature from object for the next concatenate operation
            v = self.ca_v_proj(memory) # 原文图中Multi head (Cross) Attention的context feature from object 
            k_pos = self.ca_kpos_proj(pos) # 原文图中Multi head (Cross) Attention的position embedding from object
        ...
        if is_first or self.keep_query_pos:
            ...
            q = q_content
            k = k_content
        ### q_content成为cross attention module的新query，如原文MTR decoder图中的Self Attention伸出来连到Q的地方

        ###
        query_sine_embed = self.ca_qpos_sine_proj(query_sine_embed) # dynamic_query_center in decoder of MTR !!!
        ###
        ...
        if self.use_local_attn:
            ...
            query_sine_embed = query_sine_embed.view(num_queries, bs, self.nhead, n_model // self.nhead) ### dynamic_query_center
            q = torch.cat([q, query_sine_embed], dim=3).view(num_queries, bs, n_model * 2) ### 原文图中Multi head (Cross) Attention的concatenate操作 for object
