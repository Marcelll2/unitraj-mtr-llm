...
class MultiheadAttentionLocal(nn.Module):
    ...
    def __init__(
        ...
        # 这个矩阵可以被拆分为 3 个 (embed_dim, embed_dim) 的子矩阵，分别对应查询、键和值的线性变换
    ...
    def forward(
        ...
        Shape:
            ...
            - index_pair: :math:`(N, L)` where N is the total query tokens length (equals to ``query''),
                ...
                对于每个查询（即每一行），列出的索引值指向它在键（key）中的配对位置
            - query_batch_cnt: :math:`(B)` where B indicate batch_size. 每个值表示对应批次中的查询数
            - key_batch_cnt: :math:`(B)` where B indicate batch_size. 指定了每个批次包含的键数量
            - index_pair_batch: :math:`(N)` where N is the total query tokens length (equals to ``query'') 每个值表示对应查询的批次索引
